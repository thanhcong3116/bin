...
bool removeHead(List *&L)
{
    if(L->p_head==nullptr || L->p_tail== nullptr) return false;
...
void removeBefore(List *&L, int val)
{
    ...
    if (L == nullptr || L->p_head == nullptr || L->p_head->key == val) {
        return; 
    }
...
void removePos(List *&L, int data, int pos)
{
    ...
    while (current->p_next != nullptr && index < pos - 1) {
        current = current->p_next;
        index++;
    }
